...
// ── print shim
// ──────────────────────────────────────────────────────────────── GCC 15 ships
// <print> and the has_include branch fires, but std::print is not imported into
// the anonymous namespace automatically.  Pull it in explicitly.
...
// SIG_* handlers must have C linkage signature — parameter intentionally
// unused.
...
void log_frame(std::string_view tag, const j1939::Frame& f) {
    print("{}: PGN=0x{:05X}  SA=0x{:02X}  DA=0x{:02X}  data=[", tag, f.pgn,
...
// bugprone-exception-escape: main() must not propagate exceptions to the
// runtime.
int run() {
    ...
    {
        const std::array<uint8_t, 7> payload {0xDE, 0xAD, 0xBE, 0xEF,
                                              0x01, 0x02, 0x03};
        print("[main] ECU A → ECU B: proprietary-A\n");
        if (auto r =
                ecu_a->send(Pgn::ProprietaryA, 6U, ecu_b->address(), payload);
            !r) {
    ...
    print(
        "[main] ECU A: add DM1 fault — SPN 100 (engine oil pressure), FMI 1\n");
    ...
    print("       try: cansend vcan0 18EAFFA0#CAFECA00  (DM1 request to ECU "
          "A)\n\n");
