             ...
             std::span<const uint8_t> data)
{
    ...
    if (data.empty() || data.size() > kMaxPayload) {
        return std::unexpected(std::string_view{
            "FastPacket: payload size must be in [1, 223]"});
    }
...
void Receiver::on_frame(const Id& id, std::span<const uint8_t> data)
{
    ...
    if (frame_counter == 0U) {
        ...
        if (s.total_bytes == 0U) { return; }

        s.buffer.resize(s.total_bytes);

        const size_t available = data.size() - 2U;  // bytes after header
        const size_t first_chunk = std::min<size_t>(
            std::min<size_t>(6U, available), s.total_bytes);
        ...
        const size_t remaining = (offset < s.total_bytes) ? (s.total_bytes - offset) : 0U;
        const size_t available = (data.size() > 1U) ? (data.size() - 1U) : 0U;
        const size_t chunk     = std::min<size_t>(std::min<size_t>(7U, remaining), available);
