...
AddressClaimer::AddressClaimer(can::Socket &socket, Address preferred_address,
                               Name own_name)
    : socket_{socket}, own_name_{own_name}, preferred_{preferred_address},
...
void AddressClaimer::announce() { send_address_claimed(current_); }
...
void AddressClaimer::send_address_claimed(Address sa) {
  const Id id = Id::from_pgn(static_cast<uint32_t>(Pgn::AddressClaimed), 6U,
                             kBroadcast, sa);
  ...
  for (uint8_t i = 0U; i < 8U; ++i) {
    // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index)
    f.data[i] = static_cast<uint8_t>(
        (name_enc >> (static_cast<uint64_t>(i) * 8U)) & 0xFFU);
  }
  socket_.send(f);
}
