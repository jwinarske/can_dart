...
class Sender {
  public:
    ...
    // Send a Fast Packet message. Thread-safe (internal mutex serializes
    // sends). data.size() must be in [1, kMaxPayload].
    [[nodiscard]]
    std::expected<void, std::string_view> send(uint32_t pgn, Priority priority,
                                               Address dest, Address sa,
...
class Receiver {
  ...
  private:
    ...
    static SessionKey make_key(Address sa, uint32_t pgn) {
        return (static_cast<uint64_t>(sa) << 24) |
               static_cast<uint64_t>(pgn & 0xFFFFFFU);
    }
