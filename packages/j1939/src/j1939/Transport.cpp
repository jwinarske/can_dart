...
std::array<uint8_t, 8> make_bam_cm(uint32_t pgn, uint16_t total_bytes,
...
BamSender::send(uint32_t pgn, Priority priority,
  ...
  for (uint8_t seq = 1U; seq <= total_packets; ++seq) {
    ...
    if (!socket_->send(
            make_tp_dt(sa_, kBroadcast, priority, make_bam_dt(seq, data)))) {
...
void Receiver::on_tp_cm(const Id &id, std::span<const uint8_t> data) {
  ...
  if (ctrl == TpControl::Bam) {
    ...
    s.total_bytes =
        static_cast<uint16_t>(static_cast<uint32_t>(data[1]) |
...
void Receiver::on_tp_dt(const Id &id, std::span<const uint8_t> data) {
  ...
  if (seq != s.next_packet) {
    if (!s.is_bam()) {
      send_cts(src, s.pgn, s.next_packet);
    }
    return;
  }
...
void Receiver::send_eom_ack(Address dest, uint32_t pgn, uint16_t total_bytes,
