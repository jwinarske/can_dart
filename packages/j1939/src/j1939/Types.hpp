...
struct Id {
    ...
    [[nodiscard]] uint32_t pgn() const noexcept {
        const uint32_t base = (static_cast<uint32_t>(edp) << 17U) |
    ...
    [[nodiscard]] uint32_t encode() const noexcept {
        return (static_cast<uint32_t>(priority & 0x7U) << 26U) |
               ...
               (static_cast<uint32_t>(ps) << 8U) | static_cast<uint32_t>(sa);
    }
    ...
    [[nodiscard]] static Id from_pgn(uint32_t pgn_val, Priority prio,
        ...
        const uint8_t ps_field =
            (pf_field < 0xF0U) ? dest : static_cast<uint8_t>(pgn_val & 0xFFU);
        return {
            .priority = prio,
    ...
    [[nodiscard]] static Name decode(uint64_t raw) noexcept {
        return {
            ...
            .vehicle_system_instance =
                static_cast<uint8_t>((raw >> 60U) & 0x0FU),
        };
    }
