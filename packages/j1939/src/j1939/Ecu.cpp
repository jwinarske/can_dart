...
#  include <print>
   namespace j1939 { using std::print; }
#else
#  include <cstdio>
#  include <format>
   namespace j1939 {
...
struct Ecu::Impl {
    ...
    void handle_address_claimed_frame(const Id& id, std::span<const uint8_t> data);
...
Ecu::create(std::string_view ifname, Address preferred_address, Name name)
{
    ...
    ecu->impl_ = std::make_unique<Impl>(std::move(*sock), preferred_address, name);
    auto& impl = *ecu->impl_;
    ...
    // Start the RX thread.
    impl.rx_thread_ = std::jthread{[&impl](const std::stop_token& stop) {
        impl.rx_loop(stop);
    }};
...
void Ecu::Impl::rx_loop(const std::stop_token& stop)
{
    ...
    while (!stop.stop_requested()) {
        ...
        // Use unique_lock so we can release before invoking the message handler.
        // Calling an arbitrary callback under the mutex is fragile: it would
        // deadlock if the handler ever called back into send() or any other
        ...
        if (frame_pgn == static_cast<uint32_t>(Pgn::TpCm)) {
            ...
            const std::span<const uint8_t> req_data{raw->data.data(), raw->dlc};
            if (req_data.size() >= 3U &&
        ...
        if (dm1_response_dest) { send_dm1_response(*dm1_response_dest); }
        if (user_frame)        { on_assembled_frame(*user_frame); }
    }
}
                                              ...
                                              std::span<const uint8_t> data)
{
    ...
    for (uint8_t i = 0U; i < 8U; ++i) {
        name_raw |= static_cast<uint64_t>(data[i]) << (static_cast<uint64_t>(i) * 8U);
    }
...
void Ecu::Impl::dispatch_tx_tp_cm(const Id& id,
                                    std::span<const uint8_t> data)
{
    if (data.size() < 8U) { return; }
    ...
    if (ctrl != TpControl::Cts         &&
        ctrl != TpControl::EndOfMsgAck &&
        ctrl != TpControl::Abort)       { return; }
...
void Ecu::Impl::send_dm1_response(Address dest)
{
    ...
    if (payload.size() <= kMaxDataSize) {
        ...
        asio::co_spawn(bam_strand_,
            transport::BamSender{socket, own_addr}
                .send_async(pgn, 6U, std::move(payload)),
            asio::detached);
    } else {
        // Unicast DM1 > 8 bytes: send RTS and let the RX loop drive it.
        transport::RtsCtsSession session{socket, own_addr, dest, pgn, 6U, payload};
        if (auto r = session.start(); !r) { return; }
        const std::scoped_lock lock{mutex};
...
Ecu::send(Pgn pgn, Priority priority, Address dest,
          std::span<const uint8_t> data)
{
    ...
    if (data.size() <= kMaxDataSize) {
        ...
        if (!impl_->socket.send(f)) {
            return std::unexpected(std::string_view{"send: socket write failed"});
        }
        return {};
    }
    ...
    // requires copy-constructibility) can share ownership of the sync primitives.
...

    {
        ...
        transport::RtsCtsSession session{
            impl_->socket, own_addr, dest, pgn_raw, priority,
            {data.begin(), data.end()}};
        session.set_complete_callback(
            [done, success, cv, cv_mtx](bool ok) {
                {
        ...
        if (auto r = session.start(); !r) { return r; }
        impl_->tx_sessions_.emplace(dest, std::move(session));
    }

    // Wait for EndOfMsgAck or Abort.  Timeout = T3_ResponseWait × (retries + 1).
                     ...
                     std::function<void(std::error_code)> on_complete)
{
    ...
    if (data.size() <= kMaxDataSize) {
        const auto r = send(pgn, priority, dest,
    ...
    if (dest != kBroadcast) {
        ...
        // Shared flag so the callback and timer can cancel each other exactly once.
        auto fired = std::make_shared<std::atomic<bool>>(false);

        // Timeout: T3_ResponseWait × (kMaxRetries + 1) per J1939/21 §5.10.2.3
        const auto timeout = transport::kT3_ResponseWait *
                             static_cast<int>(transport::kMaxRetries + 1U);
        auto timer = std::make_shared<asio::steady_timer>(impl_->bam_strand_, timeout);

        // Cancel the session if the timer fires before EndOfMsgAck arrives.
        timer->async_wait(asio::bind_executor(impl_->bam_strand_,
            [timer, fired, dest_addr = dest,
             impl_ptr = impl_.get(),
             cb = on_complete](const asio::error_code& ec) mutable {
                if (ec || fired->exchange(true)) { return; }
                {
...

        {
            ...
            transport::RtsCtsSession session{
                ...
                static_cast<uint32_t>(pgn), priority, std::move(data)};
            session.set_complete_callback(
                [timer, fired, dest_addr = dest,
                 impl_ptr = impl_.get(),
    ...
    asio::co_spawn(
        ...
        [bam     = transport::BamSender{impl_->socket, own_addr},
         pgn_raw = static_cast<uint32_t>(pgn),
         priority,
            ...
            -> asio::awaitable<void>
        {
            ...
            try {
                const auto result =
                    co_await bam.send_async(pgn_raw, priority, std::move(payload));
                if (!result) {
...
std::expected<void, std::string_view>
Ecu::send_request(Address dest, Pgn requested_pgn)
{
    const std::scoped_lock lock{impl_->mutex};
    ...
    const Id id = Id::from_pgn(static_cast<uint32_t>(Pgn::Request), 6U,
                                dest, impl_->own_address);
    can::RawFrame f;
    f.id  = id.encode();
    f.dlc = 3U;
    std::ranges::copy(payload, f.data.begin());
