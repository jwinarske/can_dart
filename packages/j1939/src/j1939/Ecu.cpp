  ...
  while (!stop.stop_requested()) {
    ...
    if (frame_pgn == static_cast<uint32_t>(Pgn::TpCm)) {
      ...
      if (req_data.size() >= 3U &&
        ...
        const uint32_t req_pgn = static_cast<uint32_t>(req_data[0]) |
...
void Ecu::Impl::send_dm1_response(Address dest) {
  ...
  for (const auto &fault : faults_snap) {
    ...
    payload.push_back(static_cast<uint8_t>(((fault.spn >> 16U) & 0x07U) << 5U) |
                      (fault.fmi & 0x1FU));
    payload.push_back(static_cast<uint8_t>(
        (fault.conversion_flag ? 0x80U : 0x00U) | (fault.occurrence & 0x7FU)));
  }
...
std::expected<void, std::string_view> Ecu::send(Pgn pgn, Priority priority,
  ...
  {
    const std::scoped_lock lock{impl_->mutex};
    transport::RtsCtsSession session{impl_->socket, own_addr,
                                     dest,          pgn_raw,
                                     priority,      {data.begin(), data.end()}};
