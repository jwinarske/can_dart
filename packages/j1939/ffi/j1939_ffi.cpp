...
void post_dm1(Dart_Port port, uint8_t source, uint32_t spn, uint8_t fmi,
    ...
    Dart_CObject s {Dart_CObject_kInt32,
                    {.as_int32 = static_cast<int32_t>(spn)}};
...
J1939Handle* j1939_create(const char* ifname, uint8_t preferred_address,
    ...
    // on_message fires on the C++ RX thread — Dart_PostCObject_DL is
    // thread-safe.
    ...
    // on_claim_result fires once on the RX thread when address claiming
    // settles. addr = claimed address on success; std::nullopt on failure (no
    // free address).
