                ...
                const uint8_t* src, size_t len) noexcept
{
    ...
    Dart_CObject pgn_obj { Dart_CObject_kInt32, { .as_int32 = static_cast<int32_t>(pgn) } };
...
void post_dm1(Dart_Port port, uint8_t source,
              uint32_t spn, uint8_t fmi, uint8_t occurrence) noexcept
{
...
void post_send_complete(Dart_Port port, int32_t send_id, int32_t errno_val) noexcept
{
...
int32_t j1939_last_error(void)
{
    return tl_last_error;
}

J1939Handle* j1939_create(const char* ifname,
                           ...
                           int64_t     event_port_id)
{
    ...
    // addr = claimed address on success; std::nullopt on failure (no free address).
...
void j1939_destroy(J1939Handle* handle)
{
    delete handle;
}

int32_t j1939_send(J1939Handle*   handle,
                   uint32_t       pgn,
                   uint8_t        priority,
                   uint8_t        dest,
                   const uint8_t* data,
                   uint16_t       len)
{
    auto r = handle->ecu->send(
        static_cast<Pgn>(pgn), priority, dest,
...
void j1939_add_dm1_fault(J1939Handle* handle,
                          uint32_t     spn,
                          uint8_t      fmi,
                          uint8_t      occurrence)
{
...
uint8_t j1939_address(J1939Handle* handle)
{
    return handle->ecu->address();
}
...
int32_t j1939_send_request(J1939Handle* handle,
                            uint8_t      dest,
                            uint32_t     requested_pgn)
{
    const auto r = handle->ecu->send_request(
        dest, static_cast<j1939::Pgn>(requested_pgn));
    if (!r) {
        tl_last_error = EPIPE;
        return -EPIPE;
    }
...
void j1939_send_async(J1939Handle*   handle,
                       int32_t        send_id,
                       uint32_t       pgn,
                       uint8_t        priority,
                       uint8_t        dest,
