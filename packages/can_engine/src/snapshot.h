...
struct FilterConfig {
    ...
    uint8_t    _pad[7];
    union {
        struct { double alpha; }            ema;
        struct { double max_rate_per_sec; }  rate_limit;
        struct { double deadband; }          hysteresis;
    } params{};
};
...
struct EmaState {
    ...
    double update(double input, double alpha) {
        if (!init) { state = input; init = true; return input; }
...
struct RateLimitState {
    ...
    double update(double input, double max_rate, uint64_t ts_us) {
        if (!init) { last = input; last_ts = ts_us; init = true; return input; }
...
struct HysteresisState {
    ...
    double update(double input, double deadband) {
        if (!init) { last_out = input; init = true; return input; }
        if (std::abs(input - last_out) > deadband) last_out = input;
        return last_out;
    }
};
