...
inline double apply_filters(double value, PerSignalState& state, uint64_t ts_us) {
    for (uint8_t i = 0; i < state.filter_count; ++i) {
        ...
        switch (cfg.type) {
            ...
            case FilterType::RateLimit:
                value = state.rate[i].update(value, cfg.params.rate_limit.max_rate_per_sec, ts_us);
                break;
