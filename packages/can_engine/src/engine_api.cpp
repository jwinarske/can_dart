...
void engine_load_signals(can_engine_t* e,
                         const void* signal_defs, uint32_t signal_count,
                         const void* message_defs, uint32_t message_count) {
    if (!e) return;
...
void engine_set_filter_chain(can_engine_t* e, uint32_t sig_idx,
    ...
    if (!e) return;
    e->engine.set_filter_chain(sig_idx,
                               static_cast<const FilterConfig*>(filters), count);
}
...
void engine_reset_filters(can_engine_t* e) {
    if (e) e->engine.reset_filters();
}

// ── CAN hardware filters ──

void engine_set_can_filters(can_engine_t* e,
                            const void* filters, uint32_t count) {
    if (!e) return;
    e->engine.set_can_filters(
        static_cast<const struct can_filter*>(filters), count);
}

// ── TX ──

int engine_send_frame(can_engine_t* e, uint32_t can_id,
...
void engine_stop_periodic_tx(can_engine_t* e, uint32_t can_id) {
    if (e) e->engine.stop_periodic_tx(can_id);
}

void engine_stop_all_periodic_tx(can_engine_t* e) {
    if (e) e->engine.stop_all_periodic_tx();
}
...
void engine_remove_graph_signal(can_engine_t* e, uint32_t signal_index) {
    if (e) e->engine.remove_graph_signal(signal_index);
}

// ── Display filter ──

void engine_set_display_filter(can_engine_t* e,
                               const uint32_t* pass_ids, uint32_t count) {
    if (e) e->engine.set_display_filter(pass_ids, count);
}

void engine_clear_display_filter(can_engine_t* e) {
    if (e) e->engine.clear_display_filter();
}
