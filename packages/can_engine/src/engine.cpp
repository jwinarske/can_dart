...
Engine::Engine() : busload_timer_(io_ctx_), highlight_timer_(io_ctx_) {}
...
void Engine::set_filter_chain(uint32_t sig_idx, const FilterConfig* filters,
                              uint8_t count) {
    if (sig_idx >= signal_state_.size())
        return;
...
void Engine::set_can_filters(const struct can_filter* filters, uint32_t count) {
    if (sock_fd_ < 0)
        return;
    setsockopt(sock_fd_, SOL_CAN_RAW, CAN_RAW_FILTER, filters,
...
int Engine::start_periodic_tx(uint32_t can_id, const uint8_t* data, uint8_t dlc,
    ...
    for (auto& ptx : periodic_txs_) {
        if (ptx->can_id == can_id) {
            ...
            ptx->frame.can_dlc =
                std::min(dlc, static_cast<uint8_t>(CAN_MAX_DLEN));
...
int Engine::add_graph_signal(uint32_t signal_index) {
    if (signal_index >= signal_defs_.size())
        return -1;
    if (snapshot_.graph_count >= MAX_GRAPH_SIGNALS)
        return -1;
...
int Engine::open_socket(const char* interface_name) {
    ...
    if (ioctl(fd, SIOCGIFINDEX, &ifr) < 0) {
        std::snprintf(snapshot_.error_msg, sizeof(snapshot_.error_msg),
                      "ioctl(SIOCGIFINDEX) failed for %s: %s", interface_name,
                      strerror(errno));
...
void Engine::decode_signals(uint32_t can_id, const uint8_t* data,
                            uint64_t ts_us) {
    ...
    for (uint32_t i = 0; i < mdef.signal_count; ++i) {
        ...
        if (sig_idx >= signal_defs_.size() || sig_idx >= MAX_SIGNALS)
            continue;
...
void Engine::append_frame_row(uint32_t can_id, const uint8_t* data, uint8_t dlc,
                              uint64_t ts_us) {
...
void Engine::publish_snapshot() {
    ...
    snapshot_.sequence.store(
        snapshot_.sequence.load(std::memory_order_relaxed) + 1,
...
void Engine::start_busload_timer() {
    ...
    busload_timer_.async_wait(
        [this](const asio::error_code& ec) { on_busload_tick(ec); });
}
...
void Engine::on_busload_tick(const asio::error_code& ec) {
    ...
    // Bus load estimate: assume 500 kbit/s, standard CAN frame overhead ~111
    // bits
    // + data This is a rough estimate; accurate calculation requires bitrate
    // config
    double bits_per_sec = 0;
    // Average frame: 47 bits overhead + 8*dlc data bits (standard CAN)
    if (busload_frame_count_ > 0) {
        double avg_dlc =
            static_cast<double>(busload_byte_count_) / busload_frame_count_;
...
void Engine::start_highlight_timer() {
    ...
    highlight_timer_.async_wait(
        [this](const asio::error_code& ec) { on_highlight_tick(ec); });
}
...
void Engine::format_hex(const uint8_t* data, uint8_t dlc, char* out,
                        size_t out_len) {
    size_t pos = 0;
...
void Engine::format_frame_text(uint32_t can_id, const uint8_t* data,
                               uint8_t dlc, uint64_t ts_us, char* out,
                               size_t out_len) {
    double ts_sec = static_cast<double>(ts_us) / 1e6;
    char hex[192];
    format_hex(data, dlc, hex, sizeof(hex));
    std::snprintf(out, out_len, "%10.6f  %03X  [%u]  %s", ts_sec, can_id, dlc,
                  hex);
}
